...
'id_token': This is a unique JWT token generated by Google for each logged in user. This is essentially a key 
that allows our users to access our backend services

'uid': A unique identifier for either a business, or a user. While an id_token would be randomly generated on demand this persists which allows us or other users to query businesses or get publicly available information.

'image_id': A unique name for any photo we add to our database. We don't want duplicate names being assigned to the same user as that will cause collisions in our query.

'image_base64': All photos are encoded using the base64 standard. This is useful for transfering photos from our front end to backend. 


## machine_learning.py
**/predict \[POST\] - Makes a request to AutoML returning bounding boxes of the object**
Parameters:
- files[] - A list of photos that we want to send to AutoML

Note:
Uses the form data type
## user_database.py
...
- 'data': The json containing the photo meta data (Note that this data is created by the machine learning model)
...
Parameters:
- 'id_token': JWT token authenticating the business
- 'data': A json containing the tokens we want to update (This is the same json as above)
